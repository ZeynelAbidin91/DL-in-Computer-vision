...
ap.add_argument('-d', '--dataset', required=True, 
                help='path to CALTECH Faces dataset')
ap.add_argument('-s', '--sample_size', type=int, default=10, 
                help='# of example samples')
...
# encode the labels, tranforming them from strings into integers 
le = LabelEncoder()
...
for i in np.random.randint(0, high=len(testing.data), size=(args["sample_size"],)):
    # resize the face to make it more visable, then display the facce and the prediction
