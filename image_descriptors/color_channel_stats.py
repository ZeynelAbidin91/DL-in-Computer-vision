...
image = cv2.imread('raptors_01.png')
img2 = cv2.imread('raptors_02.png')
cv2.imshow('Image', image)
(means, stds) = cv2.meanStdDev(image)
features1 = np.concatenate([means, stds]).flatten()
(means2, stds2) = cv2.meanStdDev(img2)
features2 = np.concatenate([means2, stds2]).flatten()
print(features1)
print(features2)
print(dist.euclidean(features1, features2))
'''
