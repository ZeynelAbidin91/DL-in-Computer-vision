...
def describe (image):
    # extract the mean and standart deviation from each channel of the image
...
ap.add_argument('-d', '--dataset', required=True, 
                help='path to 8 scene categoty dataset')
...
# loop over the image in the input directory
for imagePath in imagePaths:
    # extract the label and load the image from disk
    label = imagePath[imagePath.rfind('/') + 1:].split('_')[0]
    image = cv2.imread(imagePath)

    # extract the feature from the image, then update the list of labels
    # and features
