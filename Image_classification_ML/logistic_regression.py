...
# handle over versions of sklearn
if int((sklearn.__version__).split(".")[1]) < 18:
    from sklearn.cross_validation import train_test_split
else:
    from sklearn.model_selection import train_test_split

# grab a small subset if the labeled Faces in the Wild 
# dataset, then construct the training and testing splits
# (note: if this is your first time running this script
# it may take awhile for the dataset to download -- but 
# once it has downloaded the data will be cached locally 
# and subsequent runs will be substiantially faster)
print("[INFO] fetching data...")
dataset = datasets.fetch_lfw_people(min_faces_per_person=70,funneled=True, 
                                    resize=0.5)
(trainData, testData, trainLabels, testLabels) = train_test_split(dataset.data, 
            dataset.target, test_size=0.25, random_state=42)

# train the model an show the classification report
