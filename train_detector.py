...
# then initial list of images and bounding boxes used to train the classifier
...
for imagePath in paths.list_images(args['class']):
    ...
    imageID = os.path.split(imagePath)[1].split("_")[1]
	...
	# boxes
    bb = [dlib.rectangle(left=int(x), top=int(y), right=int(w),
                        bottom=int(h)) for (y, h, x, w) in annotations]
    boxes.append(bb)
