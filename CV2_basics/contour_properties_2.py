...
# find the external contours in the thresholded image and allocate
# memory for the convex hull image
...
for (i,c) in enumerate(cnts):
    ...
    # compute the aspect ratio of contour, which is simply the 
    # width divided by the height of bounding box
    aspectRatio = w / float(h)

    # use the area of contour and bounding box area to compute
    # the extent
    extent = area / float(w * h)

    # compute the convex hull of the contour, the use the area 
    # of the original contour and the area of the convex hull to 
    # compute the solidity
    hull = cv2.convexHull(c)
    hullArea = cv2.contourArea(hull)
    solidity = area / float(hullArea)

    # visualize the original contours and the convex hull and 
