...
# grab the image channels, initialize the tuple of color and the
# figure
...
for (chan, color) in zip(chans, colors):
    # createa h
    # 
    # istogram for the current channel and plot it
    hist = cv2.calcHist([chan], [0], None, [256], [0, 256])
    plt.plot(hist, color = color)
    plt.xlim([0, 256])

# let's move on to 2D histograms -- we need to reduce the 
# the number of bins in the histogram from 256 to 32 so we 
