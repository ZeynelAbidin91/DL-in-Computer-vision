...
class Vocabulary:
    ...
    def fit(self, numClusters, samplePercent, randomState=None):
        ...
        # determine the nymber of features to sample, generate the indexes
        # of the sample, sorting them in ascending order to speedup access
        # time from the HDF5 database
        sampleSize = int(np.ceil(samplePercent * totalFeatures))
        idxs = np.random.choice(np.arange(0, totalFeatures), (sampleSize),      
        replace=False)
        idxs.sort()
        data = []
        self._debug("srarting sampling...")

        # loop over the randomly sampled indexes and accumulate the features
        # to cluster
        for i in idxs:
            data.append(db["features"][i][2:])
            # cluster the data
            self._debug("sampled {:,} features from a population of {:,}"
            .format(len(idxs), totalFeatures))
