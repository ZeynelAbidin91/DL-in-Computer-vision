...
class BaseIndexer(object):
    def __init__(self, dbPath, estNumImages=500, maxBufferSize=50000,
        ...
        # store the database path, estimated number of images in the dataset,
        # maxbuffer, the resize factor of the database and the verbosity setting 
        self.dbPath = dbPath
        ...
        def _writeBuffer(self, dataset, datasetName, buf, idxName, sparse=False):
            # if the buffer is a list, then compute the index based on the lists 
            # length
            if type(buf) is list:
            ...
            # check to see ifd the dataset needs to be resized
        ...
        def _resizeDataset(self, dataset, dbName, baseSize=0, finished=0):
            ...
            # check to see if we are finished writing rows to the
            # dataset, and if so, make the new size current index
            if finished > 0:
                newSize = finished
            
            # otherwise, we re enlarging the dataset so calculate the 
            ...
            self._debug("old size of '{}': {:,}; new size: {:,}".
                        format(dbName, origSize, newSize))
